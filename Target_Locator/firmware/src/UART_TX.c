...
void TX_InterruptHandler(const SYS_MODULE_INDEX index)
{
    ...
    xHigherPriorityTaskWoken = pdFALSE;
    uint8_t byte;
    if (xQueueReceiveFromISR(UART_TX_Byte_Q, &byte, &xHigherPriorityTaskWoken)){
...
void UART_TX_Initialize ( void )
{
    ...
    //DRV_USART_ByteTransmitCallbackSet(DRV_USART_INDEX_0, TX_InterruptHandler);
}
